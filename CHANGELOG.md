...
- Interactive sign-in now lands on a branded localhost page matching the project banner, replacing azure-identity's one-line text response; a failed sign-in shows the Entra error instead of a blank success message.
...
- `devops_get_work_item_attachment` — download a work item attachment and return it to the model as viewable image content. Accepts an attachment GUID or a full attachment URL.
- `devops_upload_work_item_attachment` _(write)_ — upload an image from a local path or base64 bytes; returns the attachment reference plus ready-to-paste `embed.markdown` and `embed.html` snippets.
- `devops_list_work_item_attachments` — list every attachment a work item references, from both the Attachments tab and images embedded in its large-text fields, de-duplicated. Optional `include_comments` also scans the discussion.
- `devops_link_work_item_attachment` _(write)_ — put an uploaded attachment on the Attachments tab. Idempotent.
- `devops_get_repository_image` — read an image committed to a Git repository and return it as viewable image content.
- `devops_delete_work_item` _(delete)_ — delete a work item to the recycle bin, or permanently with `destroy=true`. Registered only under `AZDO_ALLOW_DELETE=true`.
- `AZDO_ATTACHMENT_ROOT` — optional directory tree that uploads from a local path are confined to.

### Fixed

- `devops_get_file_content` returned the Git item's metadata JSON instead of the file's contents, on every call since the tool shipped. Its refusal of binary files never fired for the same reason and now works.
- Images embedded in a work item field were not found unless their URL was absolute, so a work item could be reported as having no attachments when it visibly had one.
- HTML entities in an attachment's file name were not decoded — a file called `a & b (1).png` was reported as `a &amp; b (1).png`.
